...
*/
    ...
    class Auth extends React.Component {
        ...
        <Input type="login" name={name} value={value} onChange={this.onChange} status={this.setState.UsersLogin? 'valid' : 'error'} bottom={this.UsersLogin? 'Ваш логин успешно инициализирован' : 'Ошибка: 0x52d270. Ваши данные не внесены в систему!'} placeholder="Введите логин" />
		))}
		{this.UsersPassword.reduce(({ name, value }) => (
		<Input type="password" name={name} value={value} onChange={this.onChange} status={this.setState.UsersPassword? 'valid' : 'error'} bottom={this.UsersPassword? 'Ваш пароль успешно инициализорован!' : 'Ошибка: 0x74d270. Ваши данные не внесены в систему!'} placeholder="Введите пароль"/>
		))}
		<Button size="xl" level="primary" onClick={this.setState.UsersLogin, this.setState.UsersPassword, !this.props.go} Data-to="Home">Войти</Button>   
		</FormLayoutGroup>
        </FormLayout>
		</Group>
		</Panel>
